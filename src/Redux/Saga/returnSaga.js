...
function* amadeusSearchSaga(action) {
    ...
    try {
        const [onwardResult, returnResult] = yield all([
            call(fetchAmadeusData, `${apiURL.baseURL}/skytrails/api/combined/AMADEUSPriceSort`, onwardPayload),
            call(fetchAmadeusData, `${apiURL.baseURL}/skytrails/api/combined/AMADEUSPriceSort`, returnPayload),
        ]);

        if (onwardResult && returnResult) {
            // console.log(onwardResult, returnResult, onwardResult.data.result, "dataaa");
            const data = {
...
function* tbo_kafila_SearchSaga(action) {
    ...
    try {
        const [onwardResult, returnResult] = yield all([
            call(fetchAmadeusData, `${apiURL.baseURL}/skytrails/api/combine/combineApiRes`, onwardPayload),
            call(fetchAmadeusData, `${apiURL.baseURL}/skytrails/api/combine/combineApiRes`, returnPayload),
        ]);
