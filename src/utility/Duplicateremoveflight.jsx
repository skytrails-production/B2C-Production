...
export function findCheapestFlights(amadeusFlights, filteredFlights) {
    ...
    const extractAmadeusFlightInfo = (flight) => {
        ...
        return {
            flightNumber: flight?.flightDetails?.flightInformation?.flightOrtrainNumber || flight?.flightDetails?.[0]?.flightInformation?.flightOrtrainNumber,
            source: flight?.flightDetails?.flightInformation?.location?.[0]?.locationId || flight.flightDetails?.[0]?.flightInformation?.location?.[0]?.locationId,
            destination: flight?.flightDetails?.flightInformation?.location?.[1]?.locationId || flight?.flightDetails?.[flight?.flightDetails?.length - 1]?.flightInformation?.location?.[1]?.locationId,
            timeOfDeparture: convertAmadeusTime(
                flight?.flightDetails?.flightInformation?.productDateTime?.dateOfDeparture || flight?.flightDetails?.[0]?.flightInformation?.productDateTime?.dateOfDeparture,
                flight?.flightDetails?.flightInformation?.productDateTime?.timeOfDeparture || flight?.flightDetails?.[0]?.flightInformation?.productDateTime?.timeOfDeparture

            ),

            timeOfArrival: convertAmadeusTime(
                flight?.flightDetails?.flightInformation?.productDateTime?.dateOfArrival || flight?.flightDetails?.[flight?.flightDetails?.length - 1]?.flightInformation?.productDateTime?.dateOfArrival,
                flight?.flightDetails?.flightInformation?.productDateTime?.timeOfArrival || flight?.flightDetails?.[flight?.flightDetails?.length - 1]?.flightInformation?.productDateTime?.timeOfArrival
            ),
        }
    };
    ...
    const extractFilteredFlightInfo = (flight) => {
        ...
        const timeOfArrival = flight.ADate
            ...
            : flight.Segments
                ? convertTBOTime(flight.Segments[0]?.[flight?.Segments?.[0]?.length - 1].Destination.ArrTime)
                : "";
