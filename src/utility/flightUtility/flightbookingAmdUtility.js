...
// api for airesell
export const fetchData = async ({ sesstioResultIndex, setAirsellRes, setXmlData, adultCount, childCount, infantCount }) => {
...
export const startHBookingProcessAMD = ({ sesstioResultIndex, jsonData, adultCount, childCount, passengerData, infantCount, seatListPayload, navigate, airesellRes, finalAmount, arrTimeISO1, depTimeISO1, ResultIndex, discountvalue, jsonSavePnrData, setJsonSaveData, totalSeatAmount, }) => {
  console.log(sesstioResultIndex, "sessionResultIndex")
  const refundAmount = async ({ refundTxnId, finalAmount, totalSeatAmount }) => {
    try {
  ...
  const convertXmlToJsonSavePnr = async (res) => {
    ...
    let convertData;
    if (res !== "") {
      convertData = await result["soapenv:Envelope"]["soapenv:Body"]["PNR_Reply"];
