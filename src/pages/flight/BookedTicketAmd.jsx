...
const BookedTicket = () => {
  ...
  const { PNR, datavalue, sesstioResultIndex,finalAmount,discountvalue,arrTimeISO1,depTimeISO1,ResultIndex,jsonData} = location.state || {};
...
// console.log(reducerState?.searchFlight?.flightDetails?.parsedDate)
  ...
  useEffect(() => {
    ...
    if (!apiCalled) {
      datasaveTodb({sesstioResultIndex,finalAmount,arrTimeISO1,depTimeISO1,reducerState,jsonSavePnrData,ResultIndex,jsonData});
  ...
  let depTimeString = String(
    datavalue?.originDestinationDetails?.itineraryInfo?.travelProduct
      ?.product?.depTime
  );
...
// console.log(jsonSavePnrData?.pnrHeader?.reservationInfo?.reservation?.controlNumber)
  ...
  const times = datavalue?.originDestinationDetails?.itineraryInfo
    ...
    const PNRvalue = jsonSavePnrData?.pnrHeader?.reservationInfo?.reservation?.controlNumber;
    ...
    <div>
      ...
      >
        <div className="container">
          ...
          <p style={{ textAlign: "center", color: "#d90429", fontWeight: "700" }}>Check Your Booking details</p>
          <h2 style={{ textAlign: "center" }}>Your PNR is {PNR}</h2>

          <div className="ticketdetails col-lg-12">

          <div style={{ borderBottom: "1px solid rgba(128, 128, 128, 0.897)" }}>
                  <div style={{ padding: "40px" }}>
                    <div className="ticket-details-destination">
                      ...
                      <div>
                        <p className="tickettex">{reducerState?.searchFlight?.flightDetails?.to?.name}</p>
                      </div>
                    </div>
                  </div>
                </div>
            {datavalue?.originDestinationDetails?.itineraryInfo?.elementManagementItinerary ? (

              <>

                <div style={{ padding: "40px" }} className="row">
                  <div className="col-lg-3" style={{ borderRight: "1px solid rgba(128, 128, 128, 0.897)" }}>
                    <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                      <img
                        ...
                        alt="Airline Logo"
                      />
                      <p>{datavalue?.originDestinationDetails?.itineraryInfo?.travelProduct?.companyDetail?.identification}</p>
                    </div>
                    <div style={{ border: "1px solid #F7E3FF", display: "flex", flexDirection: "row", gap: "12px" }}>
                      <p style={{ borderRight: "1px solid #F7E3FF", padding: "5px", background: "#F7E3FF" }}>PNR</p>
                  ...
                  <div className="col-lg-9" style={{ padding: "8px" }}>
                    <div style={{ display: "flex", justifyContent: "space-evenly" }}>
                      <div>
                        <div style={{ borderBottom: "1px solid rgba(128, 128, 128, 0.897)" }}>
                          <p className="tickettexvalue">{reducerState?.searchFlight?.flightDetails?.from?.name}</p>
                          <p className="tickettexvalue">{depTime}</p>
                          <p className="tickettex1">{depDate}</p>
                        </div>
                        <div>
                          <p className="tickettex1">{findAirportByCode(datavalue?.originDestinationDetails?.itineraryInfo?.travelProduct?.boardpointDetail?.cityCode)?.code}</p>
                        </div>
                      </div>
                      ...
                      <div>
                        <div style={{ borderBottom: "1px solid rgba(128, 128, 128, 0.897)" }}>
                          <p className="tickettexvalue">{reducerState?.searchFlight?.flightDetails?.to?.name}</p>
                          <p className="tickettexvalue">{arrTime}</p>
                          <p className="tickettex1">{arrDate}</p>
                        </div>
                        <div>
                          <p className="tickettex1">{findAirportByCode(datavalue?.originDestinationDetails?.itineraryInfo?.travelProduct?.offpointDetail?.cityCode)?.code}</p>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              datavalue?.originDestinationDetails?.itineraryInfo?.map((stopss, index) => {
  ...
  const arrTime = arrivalMoment.format("HH:mm");
                return (
                  <div key={index} style={{ padding: "40px", borderBottom: "1px solid rgba(128, 128, 128, 0.897)" }}>
                   ...
                   <div className="col-lg-3" style={{ borderRight: "1px solid rgba(128, 128, 128, 0.897)" }}>
                    <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                      <img
                        ...
                        alt="Airline Logo"
                      />
                      <p>{stopss?.travelProduct?.companyDetail?.identification}</p>
                    </div>
                    <div style={{ border: "1px solid #F7E3FF", display: "flex", flexDirection: "row", gap: "12px" }}>
                      <p style={{ borderRight: "1px solid #F7E3FF", padding: "5px", background: "#F7E3FF" }}>PNR</p>
                      <p style={{ padding: "5px" }}>{PNR}</p>
                  ...
                  <div className="col-lg-9">
                    <div style={{ display: "flex", justifyContent: "space-evenly" }}>
                      <div>
                        <p className="tickettexvalue">{stopss?.travelProduct?.boardpointDetail?.cityCode}</p>
                        <p className="tickettexvalue">{depTime}</p>
                       <p className="tickettex1">{depDate}</p>
                       <p className="tickettex1">{findAirportByCode(stopss?.travelProduct?.boardpointDetail?.cityCode)?.code}</p>
                      </div>
                      ...
                      <div>
                        <p className="tickettexvalue">{stopss?.travelProduct?.offpointDetail?.cityCode}</p>
                        <p className="tickettexvalue">{arrTime}</p>
                        <p className="tickettex1">{arrDate}</p>
                        <p className="tickettex1">{findAirportByCode(stopss?.travelProduct?.offpointDetail?.cityCode)?.code}</p>
                      </div>
                    </div>
                    </div>
                  </div>
            ...
            <div style={{ borderTop: "1px solid rgba(128, 128, 128, 0.897)", background: "#F1F1F1" }}>
                  <div>
                    {reducerState?.passengers?.passengersData?.length > 0 ? (
                      <table style={{ width: "100%", borderCollapse: "collapse" }}>
                        <thead>
                          <tr>
                            <th style={{ border: "1px solid #ddd", padding: "8px" }}>TRAVELLER</th>
                            <th style={{ border: "1px solid #ddd", padding: "8px" }}>Baggage</th>
                            <th style={{ border: "1px solid #ddd", padding: "8px" }}>PNR NO.</th>
                          </tr>
                        </thead>
                        <tbody>
                          {reducerState?.passengers?.passengersData?.map((item, index) => {
                            const passenger = { ...item, gender: item?.title === "Miss" ? "Female" : "Male" };
                            return (
                              <tr key={index}>
                                <td style={{ border: "1px solid #ddd", padding: "8px" }}>
                                  {passenger.title} {passenger.firstName} {passenger.lastName}
                                </td>
                                <td style={{ border: "1px solid #ddd", padding: "8px" }}> {sesstioResultIndex?.baggage
                                      ...
                                      {sesstioResultIndex?.baggage
                                        ...
                                        : Number(
                                          ...
                                          ? "KG"
                                          : ""}</td>
                                <td style={{ border: "1px solid #ddd", padding: "8px" }}>{PNR}</td>
          ...
          <p>
  You have paid <span style={{fontWeight:"bold"}}>{(Number(finalAmount)).toFixed(2)}</span>
  {discountvalue > 0 && (
    <>
      {" "}
      You saved <span style={{fontWeight:"bold"}}>{Number(discountvalue).toFixed(2)}</span>
    </>
  )}
</p>
          </div>
