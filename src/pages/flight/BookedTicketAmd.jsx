...
const BookedTicket = () => {
  ...
  useEffect(() => {
    ...
    // const jsonResult = xmlToJson(sessionStorage.getItem("PNR"));

    console.log(sessionStorage.getItem("PNR"), "svrData");

    const fetchBookingDataAmd = async (ress) => {
      // console.log(airesellRes?.data?.MessageID, "ffffffffffffffffffffffffff");
      const res = await axios({
        method: "POST",
        url: `${apiURL.baseURL}/skyTrails/amadeus/pnrretrieve`,
        data: {
          pnr: PNR,
        },
        headers: {
