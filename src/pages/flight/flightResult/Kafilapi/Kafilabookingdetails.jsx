...
function Kafilabookingdetails() {
  ...
  const { baggage, meal, finalvalue, baggagedata, seats, mealDynamic,totalSeatAmount, pnrresponse,Destination, Origin ,discount,sesstioResultIndex} = location.state || {};
...
//  console.log(passengers,pnrresponse);
  ...
  const passengerDetails = passengers?.map((item, index) => ({
...
?.BookingInfo?.APnr;
  ...
  const addBookingDetails = async() => {
      ...
      const payloadkafila = {
        ...
        pnr: pnr,
        bookingStatus: pnrresponse?.result
        ?.BookingInfo?.APnr ? "BOOKED" : "FAILED",
        ...
        airlineDetails: sesstioResultIndex?.Itinerary?.map(
          (item, index) => {
            return {
              Airline: {
...
// console.log(finalvalue,"finalvaluefinalvaluefinalvaluefinalvalue");
  ...
  return pnr? (
    <div>
      <div
        className="tempBox"
        style={{ marginTop: "80px", marginBottom: "80px", flexDirection: "column" }}
      >
            ...
            <div style={{ borderBottom: "1px solid rgba(128, 128, 128, 0.897)" }}>
              <div style={{ padding: "40px" }}>
                <div className="ticket-details-destination">
                  <div><p className="tickettex">{sesstioResultIndex?.Itinerary[0]?.SrcName}</p></div>
                  <span className="tickettex">-</span>
                  <div><p className="tickettex"> {
            ...
            <div style={{ padding: "40px" }} className="row">
              <div className="col-lg-3" style={{ borderRight: "1px solid rgba(128, 128, 128, 0.897)" }}>
                ...
                <div style={{ border: "1px solid #F7E3FF", display: "flex", flexDirection: "row", gap: "12px" }}>
                  ...
                  <p style={{ padding: "5px" }}>{pnrresponse?.result?.BookingInfo?.APnr}</p>
                </div>
              </div>
              ...
              <div style={{display:"flex",justifyContent:"space-evenly"}}>
                <div>
                  <div style={{borderBottom:"1px solid rgba(128, 128, 128, 0.897)  "}}>
                    <p className="tickettexvalue">{sesstioResultIndex?.Itinerary?.[0]?.SrcName}</p>
                  ...
                  <div>
                    <p className="tickettex1">{sesstioResultIndex?.Itinerary?.[0]?.DArpt}</p>
                  </div>
                </div>
                ...
                <div style={{borderBottom:"1px solid rgba(128, 128, 128, 0.897)  "}}>
                  ...
                  <div>
                    <p className="tickettex1">{ sesstioResultIndex?.Itinerary[
                                  sesstioResultIndex?.Itinerary.length - 1
                                ]?.AArpt}</p>
                  </div>
                </div>
  ...
  <div>
    {passengerDetails?.length > 0 ? (
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        ...
        <tbody>
          {pnrresponse?.result?.PaxInfo?.Passengers?.map((passenger, index) => (
            <tr key={index}>
              ...
              <td style={{ border: "1px solid #ddd", padding: "8px" }}>
                {passenger?.baggage && passenger?.baggage?.length > 0 ? passenger?.baggage?.[0]?.AirlineDescription :sesstioResultIndex?.FareRule?.CHKNBG
                }
              </td>
              <td style={{ border: "1px solid #ddd", padding: "8px" }}>
                {pnr || 'N/A'}
      ...
      <p>No passenger details available.</p>
          ...
          <p>
  You have paid <span style={{fontWeight:"bold"}}>{parseInt(Number(finalvalue) )}</span>
  {discount > 0 && (
    <>
      {" "}
      You saved <span style={{fontWeight:"bold"}}>{Number(discount).toFixed(2)}</span>
    </>
  )}
</p>
          </div>
