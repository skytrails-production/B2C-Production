...
const ItenaryPdfDownloader = () => {
    ...
    useEffect(() => {
        const fetchItineraryData = async () => {
            try {
                ...
                const res = await axios.get(`${apiURL.baseURL}/skyTrails/api/itinerary/getProposalById`, {
    ...
    const arrivalDateInternational = dayjs(itineraryData?.flightData?.[0]?.flightDetails?.[0]?.Segments?.[0]?.[itineraryData?.flightData?.[0]?.flightDetails?.[0]?.Segments?.length - 1]?.Destination?.ArrTime);
    const arrivalDateDomesticReturn = dayjs(itineraryData?.flightData?.[0]?.flightDetails?.[0]?.Segments?.[0]?.[itineraryData?.flightData?.[0]?.flightDetails?.[0]?.Segments?.length - 1]?.Destination?.ArrTime);
    const arrivalDateOneway = dayjs(itineraryData?.flightData?.[0]?.flightDetails?.[0]?.Segments?.[0]?.[itineraryData?.flightData?.[0]?.flightDetails?.[0]?.Segments?.length - 1]?.Destination?.ArrTime);
    // Determine the initial date
    // const initialStartDate = getLatestDate([leavingDate, arrivalDate1, arrivalDate2]);
    const initialStartDate = getLatestDate([leavingDate, arrivalDateInternational, arrivalDateDomesticReturn, arrivalDateOneway]);
                    ...
                    <div ref={componentRef}>
                        <div className="mx-4 my-4">
                            <div className="itenaryPdfBoxHeading d-flex flex-row justify-content-between align-items-center ">
                                <div >
                                    <p className='flightPdfIteneraryPara'>Customer Name: {itineraryData?.ItenaryPayloadData?.clientName}</p>
                                    <p className='flightPdfIteneraryPara'>Journey Date: {itineraryData?.ItenaryPayloadData?.leavingDate}</p>
                                </div>
                                <div>
                                    <p className='flightPdfIteneraryPara'>leavingFrom: {itineraryData?.ItenaryPayloadData?.leavingFrom}</p>
                                    <p className='flightPdfIteneraryPara'>Nationality: {itineraryData?.ItenaryPayloadData?.nationality === "India" ? "Indian" : itineraryData?.ItenaryPayloadData?.nationality}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="mx-4">
                            ...
                            {itineraryData?.flightData?.sort((a, b) => a.id - b.id).map(flight => (
                                <div key={flight._id}>
                                    {
                                            ...
                                            <div >
                                                {flight.id == 0 ?
                                                    <p className='flightPdfIteneraryPara'>
                                                        {itineraryData?.ItenaryPayloadData?.leavingFrom} to {itineraryData?.ItenaryPayloadData?.cityAndNight?.[0]?.destination} (Round Trip)
                                                    </p> :
                                                    <p className='flightPdfIteneraryPara'> {itineraryData?.ItenaryPayloadData?.cityAndNight[flight.id - 1].destination} to {itineraryData?.ItenaryPayloadData?.cityAndNight?.[flight.id]?.destination} (Round Trip)</p>
                                                }
                                                            ...
                                                            <div class="returnFlightResultBoxOne" style={{ flex: 10 }}>
                                                                ...
                                                                <div class="returnResultOtherDetails">
                                                                    ...
                                                                    <div class="returnResultDurationBox">
                                                                            ...
                                                                            detail?.Segments[0].length > 1 ?
                                                                                <h4>
                                                                                    {`${Math.floor(
                                                                                        ...
                                                                                        60
                                                                                    )}hr ${detail?.Segments[0][0]?.Duration %
                                                                                    60
                                                                                        }min`}{" "}
                                                                                    -{" "}
                                                                                    {`${Math.floor(
                                                                        ...
                                                                        <p>{
                                                                            detail?.Segments[0].length > 1 ?
                                                                                `${detail?.Segments[0].length - 1} stop via ${detail?.Segments[0][0]?.Destination?.Airport?.CityName}` : "Non Stop"}</p>
                                                                    ...
                                                                    <div class="returnResultTimingBox">
                                                                        <span>
                                                                            {
                                                                                detail?.Segments[0][detail?.Segments[0].length - 1]?.Destination
                                                                                    ?.Airport?.CityName
                                                                            }
                                                                        </span>
                                                                        <p>
                                                                            {dayjs(
                                                                                detail?.Segments?.[0][detail?.Segments[0].length - 1]?.Destination?.ArrTime
                                                                            ).format("DD MMM, YY")}
                                                                        </p>
                                                                        <h5 className="daySize">
                                                                            {dayjs(
                                                                                detail?.Segments?.[0][detail?.Segments[0].length - 1]
                                                                                    ?.Destination?.ArrTime
                                                                            ).format("h:mm A")}
                                                                        </h5>
                                                                    </div>
                                        ...
                                        flight.flightDetails.length === 1 && (
                                            ...
                                            <>
                                                {flight.id == 0 ?
                                                    ...
                                                    <p className='flightPdfIteneraryPara'> {itineraryData?.ItenaryPayloadData?.cityAndNight[flight.id - 1].destination} to {itineraryData?.ItenaryPayloadData?.cityAndNight?.[flight.id]?.destination}</p>
                                                }
                                                {flight.flightDetails.map(detail => (
                                                    <div key={detail._id}>
                                                            ...
                                                            <div className="pdfFlightReturnBox">
                                                                ...
                                                                <div class="returnFlightResultBoxOne">
                                                                    <div class="returnResultFlightDetailsPDF">
                                                                        ...
                                                                        <span>
                                                                            {
                                                                                detail?.Segments[detail?.Segments?.length - 1][0]?.Airline
                                                                                    ?.AirlineName
                                                                            }
                                                                        </span>
                                                                        <p>
                                                                    ...
                                                                    <div class="returnResultOtherDetails">
                                                                        <div class="returnResultTimingBox">
                                                                            <span>
                                                                                {
                                                                                    detail?.Segments[detail?.Segments?.length - 1][0]?.Origin
                                                                                        ?.Airport?.CityName
                                                                                }
                                                                            </span>
                                                                            <p>
                                                                        ...
                                                                        <div class="returnResultDurationBox">
                                                                            {
                                                                                detail?.Segments[detail?.Segments?.length - 1].length > 1 ?
                                                                                    <h4>
                                                                                        {`${Math.floor(
                                                                                            detail?.Segments[detail?.Segments?.length - 1][0]?.Duration /
                                                                                            60
                                                                                        )}hr ${detail?.Segments[0][0]?.Duration %
                                                                                        60
                                                                                            }min`}{" "}
                                                                                        -{" "}
                                                                                        {`${Math.floor(
                                                                                            detail?.Segments[detail?.Segments?.length - 1][1]?.Duration /
                                                                                            60
                                                                                        )}hr ${detail?.Segments[0][0]?.Duration %
                                                                                        60
                                                                                            }min`}
                                                                                        ...
                                                                                        {`${Math.floor(
                                                                                            detail?.Segments[detail?.Segments?.length - 1][0]?.Duration /
                                                                                            60
                                                                                        )}hr ${detail?.Segments[detail?.Segments?.length - 1][0]?.Duration %
                                                                                        60
                                                                                            }min`}
                                                                                    </h4>}
                                                                            ...
                                                                            <p>{
                                                                                detail?.Segments[detail?.Segments?.length - 1].length > 1 ?
                                                                                    `${detail?.Segments[detail?.Segments?.length - 1].length - 1} stop via ${detail?.Segments[detail?.Segments?.length - 1][0]?.Destination?.Airport?.CityName}` : "Non Stop"}</p>
                                                                        ...
                                                                        <div class="returnResultTimingBox">
                                                                            <span>
                                                                                {
                                                                                    detail?.Segments[detail?.Segments?.length - 1][detail?.Segments[detail?.Segments?.length - 1].length - 1]?.Destination
                                                                                        ?.Airport?.CityName
                                                                                }
                                                                            </span>
                                                                            <p>
                                                                                {dayjs(
                                                                                    detail?.Segments?.[detail?.Segments?.length - 1][detail?.Segments[detail?.Segments?.length - 1].length - 1]?.Destination?.ArrTime
                                                                                ).format("DD MMM, YY")}
                                                                            </p>
                                                                            <h5 className="daySize">
                                                                                {dayjs(
                                                                                    detail?.Segments?.[detail?.Segments?.length - 1][detail?.Segments[detail?.Segments?.length - 1].length - 1]
                                                                                        ?.Destination?.ArrTime
                                                                                ).format("h:mm A")}
                                                                            </h5>
                                                                        </div>
                                                                        ...
                                                                        {detail?.Segments[0].length > 1 ? (
                                                                            <h4>
                                                                                ...
                                                                                {`${Math.floor(
                                                                                    detail?.Segments[0][1]?.Duration /
                                                                                    60
                                                                                )}hr ${detail?.Segments[0][1]?.Duration %
                                                                                60
                                                                                    }min`}
                                                                            </h4>
                                                                        ) : (
                            ...
                            {itineraryData?.hotelDetails?.map((item, index) => (
                                <div key={index}>
                                    <div className="headingItenary my-2">
                                        <h6>Stays in {itineraryData?.ItenaryPayloadData?.cityAndNight?.[index]?.destination} {" "} {itineraryData?.ItenaryPayloadData?.cityAndNight?.[index]?.night} {" "} Nights </h6>
                                    </div>
                                    <div className="dayWiseItenaryMainBox mb-3" style={{ backgroundColor: "#f1f1f1" }}>
                                            ...
                                            <div>
                                                {item.selectedRoom && (
                                                    ...
                                                    <p>No of Rooms {itineraryData?.ItenaryPayloadData?.RoomGuests.length}, No of Adults {itineraryData?.ItenaryPayloadData?.RoomGuests?. }</p>
                                                        ...
                                                        <p style={{ fontSize: "13px", fontWeight: "500", textAlign: "left" }} className='mb-2'>{`${itineraryData?.ItenaryPayloadData?.RoomGuests?.length} Rooms, ${adults} adults  ${children.length > 0 ? `, ${children.length} child${children.length > 1 ? 'ren' : ''}` : ''}`}</p>
                                                                        ...
                                                                        {item.selectedRoom?.Amenities && (
                                                                            <div className="othIncInner">
                                                                                <div className="d-flex justify-content-start align-items-center gap-2">
                                                                                    <p className="panDesign">{item.selectedRoom?.Amenities}</p>
                                                                                </div>
                                                                            </div>
                                                                        )}
                        ...
                        <div className="mx-4">
                            ...
                            {itineraryData?.itenerieData?.[0]?.itenararyResult?.map((itenary, itenaryIndex) => (
                                <div key={itenaryIndex} className="col-lg-12">
                                    {itenary?.[0]?.dayAt?.map((item, index) => {
                                        const dayIndex = itineraryData?.itenerieData?.[0]?.itenararyResult?.slice(0, itenaryIndex)?.reduce((acc, curr) => acc + curr?.[0]?.dayAt?.length, 0) + index;
                                        const activity = itineraryData?.itenerieData?.[0]?.activities?.[0]?.[dayIndex];
                                        ...
                                        return (
                                            <div className="dayWiseItenaryMainBox mb-3" key={index}>
                                                <div className="headingItenary">
                                                    <h6 className="mb-3">Day {dayIndex + 1} in {itenary?.[0]?.destination} {currentDate}</h6>
                                                </div>
                                                <div className="dayWiseItenaryInnerBoxPDF">
